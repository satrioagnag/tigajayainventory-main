...
$query = "SELECT rk.id, p.nama AS produk, rk.jumlah, rk.tanggal 
...
if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
    ...
    $query .= " WHERE rk.tanggal BETWEEN '$start_date' AND '$end_date'";
}

$query .= " ORDER BY rk.tanggal DESC";
$result = $conn->query($query);

// Export ke CSV
if (isset($_GET['export'])) {
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="riwayat_keluar_' . date('Y-m-d') . '.csv"');
        ...
        <div class="card filter-card">
            <div class="card-body">
                <form method="GET" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-2">
                        <a href="riwayat_keluar.php" class="btn btn-secondary w-100">
                            <i class="bi bi-arrow-counterclockwise"></i> Reset
                        </a>
                    </div>
                    <div class="col-md-2">
                        <a href="riwayat_keluar.php?export=1<?php echo isset($_GET['start_date']) ? '&start_date='.$_GET['start_date'].'&end_date='.$_GET['end_date'] : ''; ?>" 
